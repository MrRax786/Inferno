...
class Player:
    ...
    def shield_cheat(self,event):
        ...
        global shield
        shield = True
        sh_image = main_canvas.create_image(400, 750, image=sh, anchor='nw', tag="shield", state='normal')
...
class GameStartUp:
    ...
    def __init__(self):
        ...
        self.startBtn = Button(main_canvas, text="START GAME", bg='black', fg='red', font=('chiller', 20),command=self.info_screen)
        self.place = self.startBtn.place(x=525, y=250)

    def info_screen(self):
        #Screen 2: Info screen is after the menu and shows the controls to the user.
        self.startBtn.destroy()
        main_canvas.delete(ALL)
        main_canvas.create_text(main_canvas.winfo_width() / 2, 100, font=('chiller', 25), text="ARE YOU READY FOR THE INFERNO?", fill="red",tag="info")
        main_canvas.create_text(main_canvas.winfo_width() / 2, 300, font=('chiller', 7),text="CONTROLS: (A,W,S,D) TO MOVE PLAYER,(SPACE) TO SHOOT ,(Q/b) TO ACTIVATE BOSS KEY AND (P) OR (I) FOR A SURPRISE", fill="red", tag="infotxt")


        self.infoBtn = Button(main_canvas, text="CONTINUE", bg='black', fg='red', font=('chiller', 15), command= self.nameEnter)
        self.infoBtn.place(x=600, y=400)
    ...
    def nameEnter(self):
        ...
        main_canvas.delete(ALL)
        main_canvas.create_text((main_canvas.winfo_width() / 2) - 25, 100, font=('chiller', 35), text="ENTER NAME", fill="red",tag="entername")

        self.name_bx = Entry(window)
        main_canvas.create_window(600, 300, window=self.name_bx)
        self.name_bx.place(x=525,y=400)
        self.nameBtn = Button(main_canvas, text="CONTINUE", bg='black', fg='red', font=('chiller', 15), command= self.selectPlayer)
        self.nameBtn.place(x=600, y=400)
    ...
    def difficulty(self):
        ...
        main_canvas.delete(ALL)
        main_canvas.create_text(main_canvas.winfo_width() / 2, 100, font=('chiller', 35), text="CHOOSE DIFFICULTY", fill="red",tag="difficulty")
        self.easyBtn = Button(main_canvas,width= 20 ,text="EASY ☠", bg='black', fg='red', font=('chiller', 20,),command= self.easy_difficulty)
        self.easyBtn.place(x=300, y=200)
        self.mediumBtn = Button(main_canvas,width= 20, text="MEDIUM ☠☠", bg='black', fg='red', font=('chiller', 20), command= self.medium_difficulty)
        self.mediumBtn.place(x=300, y=370)
        self.hardBtn = Button(main_canvas,width= 20 , text="HARD ☠☠☠", bg='black', fg='red', font=('chiller', 20), command= self.hard_difficulty)
        self.hardBtn.place(x=300, y=550)
...
def game_over():
    ...
    main_canvas.delete(ALL)
    main_canvas.configure(bg='black')
    main_canvas.create_text(main_canvas.winfo_width() / 2, 200, font=('chiller', 50), text="GAME OVER", fill="red",tag="gameover")
    ...
    scoreplace = 500
    main_canvas.create_text(main_canvas.winfo_width() / 2, 400, font=('chiller', 30),text="LEADERBOARD", fill="red", tag="leaderboard")
    for i in leaderboard:
        main_canvas.create_text(main_canvas.winfo_width() / 2, scoreplace, font=('chiller', 10), text=i, fill="red",
                                tag="socres")
        scoreplace -= 5
